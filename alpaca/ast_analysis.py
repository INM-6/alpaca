...
The object that was input to the function is the first element of the list
`spiketrains`, that is an attribute from the first element of the list
`segments` from the `block` object. The `_CallAST` object takes the AST
associated with the statement, and all these attribute/indexing operations
are reconstructed, so that the actual input to the function is described
with respect to its hierarchical membership to the `block` object. The
attribute/indexing operations are added as individual `AnalysisStep` tuples
to the provenance history.
...
class _NameAST(ast.NodeTransformer):
    ...
    hasher : object_hash.ObjectHasher
        Instance to the class used to hash the objects.
    """
...
def _fetch_object_tree(node, time_stamp):
    ...
    # Python objects that are represented in runtime, building a
    # hierarchical tree
